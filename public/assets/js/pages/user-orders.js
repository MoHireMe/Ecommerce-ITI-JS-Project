...
document.addEventListener('DOMContentLoaded', async () => {
  ...
  try {
    ...
    if (userOrders.length === 0) {
      ...
      userOrders.sort((a, b) => getDateObjectFromISO(b.createdAt) - getDateObjectFromISO(a.createdAt));
...
async function showOrderDetailsDialog(order, products) {
  ...
  orderSummary.innerHTML = `
    ...
    <p><strong>Status:</strong> <span class="order-status status-${(order.status || 'pending').toLowerCase()}">${order.status || 'Pending'}</span></p>
  ...
  ['Product ID', 'Product Name', 'Quantity', 'Price', 'Total', 'Actions'].forEach(headerText => {
  ...
  for (const product of products) {
    ...
    const productPrice = parseFloat(product.priceAtPurchase || product.price || 0);
    ...
    reviewButton.addEventListener('click', () => {
      showReviewForm(product.productId, productName, productDialog, order, products);
    });
...
function showReviewForm(productId, productName, productDialog, order, products) {
  ...
  submitButton.addEventListener('click', async () => {
    ...
    try {
      ...
      document.getElementById('close-review-dialog').addEventListener('click', () => {
        productDialog.close();
      });
